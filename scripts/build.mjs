...
function jsonld(page, extra = []) {
  ...
  const crumbs = { '@type': 'BreadcrumbList', itemListElement: [
    ...
    ...(page.path === '/' ? [] : [{ '@type': 'ListItem', position: 2, name: page.h1, item: canon(page.path) }]),
  ] };
  ...
  const graph = [org, site, webpage, crumbs, ...extra];
...
<nav aria-label="Main"><ul>${NAV.map(n => `<li><a href="${href(n.href)}"${n.href === page.path ? ' aria-current="page"' : ''}>${esc(n.label)}</a></li>`).join('')}</ul></nav>
</div></header>
<main><div class="wrap">
${page.path === '/' ? '' : `<div class="crumbs"><a href="${href('/')}">Home</a> › ${esc(page.h1)}</div>`}
${body}
</div></main>
<footer class="site"><div class="wrap">
<div class="score">
  <div><div class="big">${snap.trustScore}<span style="font-size:18px">/5</span></div><div>Trustpilot TrustScore</div><div class="small">${snap.reviewCount.toLocaleString('en-GB')} reviews, captured ${fmtDate(snap.captured)}</div></div>
  <div class="bars">${[5, 4, 3, 2, 1].map(s => `<div><span>${s} star</span><i style="width:${pct[s] * 1.5}px"></i><span>${pct[s]}%</span></div>`).join('')}</div>
  <div style="max-width:420px"><p style="margin:0 0 8px">${esc(snap.note)}</p><a href="${SITE.trustpilotUrl}" rel="nofollow noopener" target="_blank">Read all ${snap.reviewCount.toLocaleString('en-GB')} reviews on Trustpilot →</a></div>
</div>
<p>${esc(DISCLOSURE.short)} <a href="${href('/about/')}">How this site works</a>.</p>
...
function pageBody(page) {
  const list = reviews.filter(page.filter).slice(0, page.limit);
  const topics = NAV.filter(n => !['/faq/', '/about/', '/all-reviews/'].includes(n.href) && n.href !== page.path);
  ...
  if (list.length) html += `<h2>${page.path === '/all-reviews/' ? `${list.length} reviews` : 'What customers say'}</h2><div class="grid">${list.map(reviewCard).join('\n')}</div>`;
  ...
  html += `<div class="topics">${topics.map(t => `<a href="${href(t.href)}">${esc(t.label)} reviews</a>`).join('')}${page.path !== '/all-reviews/' ? `<a href="${href('/all-reviews/')}">All reviews</a>` : ''}</div>`;
  return html;
}
...
for (const page of PAGES) {
  ...
  await writeFile(path.join(dir, 'index.html'), layout(page, pageBody(page)));
  urls.push(canon(page.path));
}
await writeFile(path.join(OUT, '404.html'), layout({ path: '/404', title: 'Page not found | Mystery Shirt in a Box Reviews', description: 'That page does not exist. Head back to the reviews.', h1: 'Page not found' }, `<h1>Page not found</h1><p>That page does not exist. <a href="${href('/')}">Back to the reviews</a>.</p>`));
